...
const Form = () => {
  ...
  const validatePage1 = () => {
    const { password, confirmPassword, fullName } = formData;
    const errors = [];
    // Validate User Full Name
    if (!fullName) {
      errors.push("Please enter your full name.");
    } else if (fullName.length < 2) {
      errors.push("Fullname required minimum 2 characters.");
    }
  ...
  const validatePage2 = () => {
    ...
    const mailformat = /^[a-zA-Z]\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,4}$/;
    // Validate User Name
    if (!userName) {
      errors.push("Please enter a username.");
    } else if (userName.length < 2) {
      errors.push("Username required minimum 2 characters.");
    }
    // Validate User Email
    if (!email) {
    ...
    if (!phone) {
      ...
      errors.push("Please enter 10 digit phone number.");
    }
    return errors.join("\n"); // No validation error
  };
  ...
  const validatePage3 = () => {
    const { nationality, city, pincode } = formData;
    ...
    if (!pincode) {
      ...
      errors.push("Please enter 6 digit pincode.");
    }
    return errors.join("\n"); // No validation error
  };
  ...
  const validateAndSubmit = (e) => {
    ...
    if (page === FormTitle.length - 1) {
      ...
      localStorage.clear();
      // setFormData(formData); // Reset the form data to initial values
    ...
    <div className="form">
      ...
      <div>
        ...
        <Modal show={diaplayModal} onHide={handleCloseModal} centered>
          ...
          <Modal.Body className="text-left font-weight-bold text-danger">
            {showSuccessMessage ? (
